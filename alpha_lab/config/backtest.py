...
def _normalize_freq(value: RebalanceFreq) -> str:
    ...
    if isinstance(value, str):
        if value not in BacktestFreq.all_values():
            raise ValueError(
                f"rebalance_freq must be one of {BacktestFreq.all_values()}, got {value!r}"
            )
        return value
    raise TypeError(f"rebalance_freq must be str or BacktestFreq, got {type(value).__name__}")
...
def _normalize_price_field(value: PriceFieldType) -> str:
    ...
    if isinstance(value, str):
        if value not in PriceField.all_values():
            raise ValueError(f"price_field must be one of {PriceField.all_values()}, got {value!r}")
        return value
    raise TypeError(f"price_field must be str or PriceField, got {type(value).__name__}")
